...
export class PrimediceSite implements DiceSite {
  name: string = 'Primedice';
  private baseUrl: string = 'https://api.primedice.com/api/v1'; // Hypothetical API base URL

  async login(apiKey: string): Promise<boolean> {
    logger.info(`Attempting to log in to Primedice with API key: ${apiKey}`);
    try {
      ...
      if (response.status === 200 && response.data.user) {
        logger.info(`Login successful for Primedice.`);
        return true;
      }
  ...
  async getBalance(apiKey: string): Promise<number> {
    logger.info(`Fetching balance from Primedice with API key: ${apiKey}`);
    try {
      ...
      if (response.status === 200 && response.data.balance) {
        logger.info(`Primedice balance fetched: ${response.data.balance}`);
  ...
  async placeBet(apiKey: string, amount: number, betRule: 'over' | 'under', betChance: number, clientSeed: string, serverSeed: string): Promise<{ success: boolean; win: boolean; profit: number; }> {
    logger.info(`Placing bet on Primedice: Amount=${amount}, Rule=${betRule}, Chance=${betChance}, ClientSeed=${clientSeed}, ServerSeed=${serverSeed}`);
    try {
      ...
      if (response.status === 200 && response.data.bet) {
        ...
        logger.debug(`Primedice bet result: Win=${win}, Profit=${profit}`);
  ...
  async withdraw(apiKey: string, amount: number, currency: string, address: string): Promise<{ success: boolean; message?: string; transactionId?: string; }> {
    logger.info(`Attempting to withdraw ${amount} ${currency} to ${address} from Primedice with API key: ${apiKey}`);
    try {
